...
ValueType _get(KeyType key, bool* found) {
    for (int ix = 0; ix < CACHE_SIZE; ix++) {
        if (cache[ix].valid && cache[ix].key == key) {
            ...
            *found              = true;
            cache[ix].last_used = ++usage_counter; // keeps track of what is being used
...
void _insert(KeyType key, ValueType value) {
    ...
    if (current_cache_size < CACHE_SIZE) {
        ...
        unsigned long long min_usage = ~0ULL;
        for (int i = 0; i < CACHE_SIZE; i++) {
            if (cache[i].valid && cache[i].last_used < min_usage) {
                min_usage = cache[i].last_used;
                index     = i;
            }
        }
    ...
    cache[index].last_used = ++usage_counter;
}
