...
void _insert(KeyType key, ValueType value) {
    ...
    if (current_cache_size < CACHE_SIZE) {
        ...
        int min_use = __INT_MAX__;
        for (int i = 0; i < CACHE_SIZE; i++) {
            if (cache[i].valid && cache[i].use_count < min_use) {
    ...
    cache[index].use_count = 1;  // Initialize use count
}
